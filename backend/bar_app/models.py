...
class Beer(models.Model):
    ...
    tap = models.IntegerField(validators=[MinValueValidator(0), MaxValueValidator(12)])
    batch_id = models.CharField(max_length=255) # this will store the batch id from BF
    is_finished = models.BooleanField(default=False)
    date_added = models.DateField(auto_now=False, auto_now_add=True)
    date_finished = models.DateField(auto_now=False, auto_now_add=False, null=True, blank=True)
    quantity_start = models.IntegerField(validators=[MinValueValidator(0)]) #quantity in oz
